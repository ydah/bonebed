...
module Bonebed
  class Dig
    ...
    def initialize(results_dir: "results", timeout: 30, offline: false)
    ...
    def run(name, phase: "require", version: nil)
      validate!(name, phase)
      manifest = phase == "install" ? install(name, version) : require_gem(name, version)
    ...
    def require_gem(name, version)
      ...
      manifest(name, specification.version.to_s, "require", collector.snapshot(normalizer))
    end
    ...
    def install(name, version)
      Dir.mktmpdir("bonebed-install-") do |root|
        ...
        manifest(name, installed_version, "install", collector.snapshot(normalizer))
      end
    end
    ...
    def manifest(name, version, phase, observation)
      ...
      files[:notable] = (files[:read].grep(/\A\$HOME\//) + files[:write].grep(/\A(?:\$HOME|\$TMPDIR)\//)).uniq.sort
      {
        ...
        "phase" => phase,
        "environment" => {"ruby" => RUBY_VERSION, "arch" => RbConfig::CONFIG.fetch("host_cpu"), "kernel" => `uname -r`.strip, "baseline_id" => nil},
        "files" => stringify_keys(files),
        "network" => counted_entries(observation.fetch(:network)),
        "exec" => counted_entries(observation.fetch(:exec)),
        "stats" => stringify_keys(observation.fetch(:stats).merge(openat_after_baseline: observation.dig(:stats, :openat_total))),
